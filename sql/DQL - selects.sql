...
SELECT evento.nome, tipoatividade.nome, atividade.titulo, atividade.data, CONCAT( sala.nome," - ", sala.numero ) AS sala, pessoa.nome, pessoa.cpf, pessoa.telefone, participante.email
FROM atividade
...
WHERE pessoa.idPessoa = 1 AND evento.idEvento = @evento
...
SELECT evento.nome AS Evento, tipoatividade.nome AS `Tipo da atividade`, atividade.titulo AS `Atividade`,
		atividade.data, CONCAT( sala.nome," - ", sala.numero ) AS sala, local.nome AS `Local`, pessoa.nome AS palestrante, atividade.descricao 
FROM atividade
INNER JOIN sala ON sala.idSala = atividade.Sala_idSala
INNER JOIN tipoatividade ON tipoatividade.idTipoAtividade = atividade.TipoAtividade_idTipoAtividade
INNER JOIN evento ON evento.idEvento = atividade.Evento_idEvento
...
#	2.10) Lista com nome, função e contato de todos os organizadores vinculados a um evento; Observações:
